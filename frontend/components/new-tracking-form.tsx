...
export function NewTrackingForm({
  ...
  const loadShipments = useCallback(async () => {
    ...
    try {
      ...
      setShipments([]);
      setShipmentsError(e instanceof Error ? e.message : "Could not load shipments");
    } finally {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      await createTrackingRequestAction({
        ...
        run_sync: true,
        ...(shippingLine.trim() ? { shipping_line: shippingLine.trim() } : {}),
  ...
  return (
    <form onSubmit={submit} className={`${formSurfaceClass} ${formClassName ?? ""}`.trim()}>
      ...
      <button
        type="submit"
        disabled={loading || (shipmentMode === "existing" && (shipmentsLoading || shipments.length === 0))}
