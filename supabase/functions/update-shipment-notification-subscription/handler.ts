...
import { updateShipmentNotificationSubscriptionQuery } from "@supabase-shared/shipment-operations.service.ts";
...
export async function handle(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (typeof body.subscribed !== "boolean") {
      return jsonResponse({ error: "subscribed must be a boolean" }, { status: 400 });
    }
    ...
    const subscribed = await updateShipmentNotificationSubscriptionQuery(userClient, {
      shipmentId,
      organizationId,
      userId: auth.userId,
    ...
    return jsonResponse({ ok: true, subscribed });
  } catch (e) {
