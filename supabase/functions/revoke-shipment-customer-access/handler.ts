...
import { revokeShipmentCustomerAccessQuery } from "@supabase-shared/shipment-operations.service.ts";
...
export async function handle(req: Request): Promise<Response> {
  ...
  try {
    const body = (await req.json()) as { access_id?: string };
    ...
    await revokeShipmentCustomerAccessQuery(userClient, accessId);
