...
export async function uploadProfileImageAndSetPath(
  ...
  assertProfileImageFile({ size: input.file.size, type: input.file.type });
...
export async function clearProfileImagePath(
  supabase: SupabaseClient,
...
> {
  if (typeof input.role !== "string" || !ALLOWED_ROLES.has(input.role)) {
