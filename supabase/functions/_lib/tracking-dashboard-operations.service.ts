...
import { buildPerformanceInsights } from "@supabase-shared/shipment-metrics";
...
export interface PersonalTriageBase {
...
/** Dashboard bundle for personal triage snapshot (`get-tracking-dashboard`). */
export interface TrackingDashboardSnapshot {
  currentUserId: string | null;
  requests: TrackingRequest[];
  alerts: Alert[];
  triageContainersById: Record<string, ContainerRow>;
...
export async function loadPersonalTriageBase(
  ...
  if (containerIds.length > 0) {
    const { data: contRows } = await supabase
      ...
      .in("id", containerIds);
    for (const row of (contRows ?? []) as ContainerRow[]) {
      map[row.id] = row;
    }
  }
...
export async function resolveOrgDashboardAccess(
  ...
  const { data: profile } = await supabase
    ...
    .eq("id", userId)
    .maybeSingle();
  const isSuperAdmin = isSuperadminRole(profile?.role as string | null | undefined);
...
async function buildOrgPerformanceInsights(
  supabase: SupabaseClient,
  organizationId: string,
  delayedCarrierLines: DelayCarrierLineRow[],
...
async function buildOrgDashboardInsights(
  ...
  const [{ data: shipmentRows }, { data: alertRows }, { data: messageRows }] = await Promise.all([
    ...
    supabase
      .from("alerts")
      .select("alert_type, severity, acknowledged_at, created_at, inbox_kind")
      .eq("organization_id", organizationId)
      .order("created_at", { ascending: false })
      .limit(500),
    supabase
...
export async function fetchOperatorTrackingRequestsPage(
  ...
  if (error) throw new Error(error.message);
  return { rows: (data as Record<string, unknown>[]) ?? [], totalCount: count ?? 0 };
}

export { normalizeOperatorSortColumn, type OperatorRequestScope, type OperatorRequestSortColumn, type SortDirection };
