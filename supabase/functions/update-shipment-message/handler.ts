...
import { updateShipmentMessage } from "@services/workspace/workspace.service.ts";
import type { UpdateShipmentMessageBody } from "@shared/dto/workspace.dto.ts";
...
export async function handle(req: Request): Promise<Response> {
  ...
  try {
    ...
    const body = (await req.json()) as UpdateShipmentMessageBody;
    const messageId = body.message_id?.trim() ?? "";
    const text = typeof body.body === "string" ? body.body : "";
    if (!messageId || !UUID_RE.test(messageId)) return jsonResponse({ error: "Invalid message_id" }, { status: 400 });
    const message = await updateShipmentMessage(userClient, auth.userId, messageId, text);
