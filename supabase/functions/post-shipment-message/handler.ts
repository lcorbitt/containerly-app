...
import { postShipmentScopeMessageWithAttachmentsForUser } from "@supabase-shared/workspace-operations.service.ts";
...
export async function handle(req: Request): Promise<Response> {
  ...
  try {
    ...
    const result = await postShipmentScopeMessageWithAttachmentsForUser(userClient, auth.userId, auth.emailLower || null, {
      organizationId, shipmentId, body, internalOnly, replyParentId, files,
    });
