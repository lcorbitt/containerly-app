-- ETD/ETA date-range filters for customer My Shipments overview (parity with operator list).
...
begin
  ...
  return query execute format($sql$
    ...
    filtered_ids as (
      ...
      where (
          ...
          or (
            ...
            and s.estimated_arrival_at::date >= $3
          )
        )
        and (
          $4::date is null
          or (
            s.estimated_arrival_at is not null
            and s.estimated_arrival_at::date <= $4
          )
        )
        and (
          $5::date is null
          or (
            ...
            and s.estimated_departure_at::date >= $5
          )
        )
        and (
          $6::date is null
          or (
            s.estimated_departure_at is not null
            and s.estimated_departure_at::date <= $6
          )
        )
        and (
          $2::text is null
    ...
    order by r.rn
  $sql$, v_order, v_dir)
  using p_customer_user_id, v_pattern, p_eta_from, p_eta_to, p_etd_from, p_etd_to, v_offset, v_limit;
end;
$fn$;
