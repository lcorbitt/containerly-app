-- Allow operators to sort the shipments overview table by any visible column.
-- Extends operator_shipments_overview_page's sort whitelist and adds an assignee
-- display-name join so the "Assignee" column sorts by name. Signature unchanged.
...
begin
  -- Keep in sync with OPERATOR_SHIPMENT_SORT_COLUMNS (frontend/utils/operator-shipment-sort.ts).
  if v_sort not in (
    'last_sync_at',
    'created_at',
    'order_number',
...
comment on function public.operator_shipments_overview_page is
  'Paged operator shipment overview with tracking rows; supports sorting by any visible list column (incl. assignee display name).';
