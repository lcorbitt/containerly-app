-- Shipments overview: return tags, search by tag, sort by tag (lexicographic tag list).
...
begin
  ...
  return query execute format($sql$
    ...
    order by r.rn
  $sql$, v_order, v_dir)
  using p_organization_id, v_scope, p_user_id, v_pattern, v_offset, v_limit, v_tag_filter;
end;
$fn$;

comment on function public.operator_shipments_overview_page is
  'Paged operator shipment overview with tracking rows; supports sorting by visible columns incl. tags.';
