-- Paged overview for customer "Shipments shared with me" (granted via shipment_customer_access).

create or replace function public.importer_granted_shipments_overview_page(
...
begin
  -- Keep in sync with IMPORTER_GRANTED_SHIPMENT_SORT_COLUMNS (frontend/utils/importer-shipment-sort.ts).
  if v_sort not in (
    'last_sync_at',
    'created_at',
    'order_number',
...
comment on function public.importer_granted_shipments_overview_page is
  'Paged customer shipment overview for active shipment_customer_access grants.';
